...
export const AuthProvider = ({ children }) => {
    ...
    const loginUser = async (nationalid, password) => {
        const response = await fetch(`${import.meta.env.VITE_APP_HTTP_BASE}://${import.meta.env.VITE_APP_URL_BASE}/Signup/token/`, {
            method: "POST",
            headers: {
    ...
    const registerUser = async (firstname, lastname, nationalid, phonenumber, password, password2, schoolname, schooltype, edulevel, province, city, address) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_HTTP_BASE}://${import.meta.env.VITE_APP_URL_BASE}/api/register/`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
